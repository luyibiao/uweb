<template>
	<div>
		...
		<!--</div>-->
		<div class="m-t">
			<auth perm="weixin.store_rule.update">
			...
			<el-dialog class="ukao-dialog"
			  ...
			  <div>
                ...
                <el-form-item prop="giftAmount" :show-message="false" style="display: inline-block" :rules="{validator: checkAmount, min:1, max:10000000, ignoreEmpty:!chkSendAmt, message: '无效的金额,保留二位小数', trigger: 'blur'}">
						   	<span v-show="chkSendAmt">
									<span class="dis-inline w-sm m-l-sm m-r-sm"><el-input type="number" v-model="ruleForm.giftAmount"></el-input></span>元
								</span>
						    </el-form-item>
					  	</div>
					  	<div class="m-t-sm">
					  		<el-checkbox v-model="chkCoupon">送优惠券</el-checkbox>
								<div v-show="chkCoupon">
									<div  class="m-b-sm" style="margin-left: 23px;" v-for="(couponItem, index) in ruleForm.couponList">
                ...
                <el-form-item prop="giftPoint" :show-message="false" style="display: inline-block"
						    ...
						    <span v-show="chkPoint">
									<span class="dis-inline w-sm m-l-sm m-r-sm"><el-input type="number" v-model="ruleForm.giftPoint"></el-input></span>分(值在1~50000之间)
								</span>
						    </el-form-item>
					    </div>
					    <div class="m-t-sm">
      ...
      submitForm() {
        ...
        this.$refs['ruleForm'].validate((valid) => {
          if (valid) {
            ...
            this.$store.dispatch('post', {
              uri: this.ruleForm.id?'userRechargeRule/update':'userRechargeRule/add',
              data: {...ruleForm}
            }).then((res)=>{
