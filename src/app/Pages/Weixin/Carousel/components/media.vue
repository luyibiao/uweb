<template>
  <div>
    <p class="text-center m-b">视频图片</p>
    <el-row class="padder-v b-t b-b">
      ...
      <el-col :span="6">
        <el-button v-if="list && list.length < 10"
                   size="small"
                   type="primary"
                   @click="openDialog(null)">添加视频/图片</el-button>
      </el-col>
    </el-row>
    ...
    <el-table class="ukao-el-table"
      ...
      <el-table-column label="图片/视频">
        <template slot-scope="scope">
          ...
          <video v-else width="128px" :poster="scope.row.posterPath"
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        imgPath: { required: true, message: "请上视频图片" },
        sortNo: [
          {
            validator: (rule, value, callback) => {
              if (value) {
  ...
  methods: {
    ...
    changeStatus() {
      msg.confirm({
        msg: `确认${this.openStatus ? "启用" : "停用"}视频图片？`,
        confirmFn: _ => {
