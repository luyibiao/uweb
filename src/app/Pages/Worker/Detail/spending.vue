...
<template>
  <div>
    <TimeTab v-model="searchDate" @change="updateData"></TimeTab>
    <el-table v-loading="loading" class="ukao-el-table m-t" :data="tableData" style="width: 100%">
      ...
      <el-table-column label="订单金额（元）" min-width="100">
        <template slot-scope="scope">{{scope.row.paymentAmount | amtFormat}}</template>
      </el-table-column>
      <el-table-column label="提成" min-width="90">
        <template slot-scope="scope" v-if="scope.row.profitType">
          <p v-if="scope.row.profitType === 1">
            {{(scope.row.profitValue || 0)/100}}%
          </p>
          <p v-else>
            {{(scope.row.profitValue || 0) | amtFormat}}元
              <span v-if="scope.row.cnt && scope.row.cnt > 1">x{{scope.row.cnt}}</span>
           </p>
        </template>    
      </el-table-column>
      <el-table-column label="收入（元）" min-width="106">
        <template slot-scope="scope">
          <span class="text-success" v-if="scope.row.income">+{{scope.row.income | amtFormat}}</span>
        </template>
      </el-table-column>
