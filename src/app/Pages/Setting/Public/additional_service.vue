<template>
	<div>
		<div class="clearfix text-right">
			<span id="public-data" class="text-md pull-left l-h36">附加服务（公共数据）</span>
		...
		<div class="m-t-vl claerfix text-right">
			<span id="private-data" class="text-md pull-left l-h36">附加服务（私有数据）</span>
		  <div class="w dis-inline m-l m-r-lg">
			...
			<auth perm="setting.public.data.project.update">
		  	<el-button @click="addForm('ruleForm')" type="primary"><i class="el-icon-plus m-r-xs"></i>添加附加服务</el-button>
			</auth>
		</div>
		<div class="m-t-lg">
			<el-table class="ukao-el-table"
		  ...
		  <div>
		  	<el-form :model="ruleForm" ref="ruleForm" label-width="160px">
				  ...
				  <el-form-item
				    ...
				    prop="name"
				    :rules="[
				      { required: true, message: '请填写附加服务名称'},
	...
	export default {
		...
		methods: {
			...
			templateList (start){
				...
				this.$store.dispatch('getTemplateList', {
					uri : 'productAddService/template/list',
					params: {
						startRow: start || 0,
			...
			updateStatusEnable (){		//启用
					...
					this.$store.dispatch('batch', {
						uri: 'productAddService/enableBatch',
						params: {
							ids: ids
						}
					}).then((res) => {
			...
			updateStatusDisable (){		//停用
				...
				let ids = this.selectionIds(list)
					this.$store.dispatch('batch', {
						uri: 'productAddService/disableBatch',
						params: {
							ids: ids
						}
					}).then((res) => {
			...
			updateStatusDelete (){		//删除
				this.$confirm('此操作将永久删除选中项, 是否继续?', '提示', {
					...
					let ids = this.selectionIds(list)
						this.$store.dispatch('batch', {
							uri: 'productAddService/deleteBatch',
							params: {
								ids: ids
							}
						}).then((res) => {
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
					if (valid) {
						this.$store.dispatch('post', {
							uri: this.ruleForm.id ? 'productAddService/update': 'productAddService/add',
