<template>
	<div>
		<div class="clearfix text-right">
			<span id="public-data" class="text-md pull-left l-h36">品牌（公共数据）</span>
		...
		<div class="m-t-lg">
		    ...
		    <el-table-column
		      ...
		      <template slot-scope="scope">
		      	<img class="thumb" :src="scope.row.brandImg || '/static/img/brand/def.png' " alt="">
		      </template>
		    </el-table-column>
		    <el-table-column
		      label="品牌名称">
		...
		<div class="m-t-vl claerfix text-right">
			<span id="private-data" class="text-md pull-left l-h36">品牌（私有数据）</span>
		  <div class="w dis-inline m-l m-r-lg">
			...
			<auth perm="setting.public.data.project.update">
		  	<el-button @click="addForm('ruleForm')" type="primary"><i class="el-icon-plus m-r-xs"></i>添加品牌</el-button>
			</auth>
		</div>
		<div class="m-t-lg">
			<el-table class="ukao-el-table"
		  	...
		  	<el-form :model="ruleForm" ref="ruleForm" label-width="160px">
				  <el-form-item
				    ...
				    :rules="[
				      ...
				      { min: 1, max: 20, message: '长度在1到20个字', trigger: 'blur' }
				    ]">
					  <div class="w-lg">
				  ...
				  <el-form-item label="图片">
						<Uploader
							...
							tip="尺寸为246像素*224像素，大小不超过2M"
							:path="ruleForm.brandImg"
		...
		methods: {
			...
			templateList (start){
				...
				this.$store.dispatch('getTemplateList', {
					uri : 'productBrand/template/list',
					params: {
						startRow: start || 0,
        ...
        this.$refs[formName].validate((valid) => {
					if (valid) {
						this.$store.dispatch('add', {
							uri: this.ruleForm.id ? 'productBrand/update': 'productBrand/add',
			...
			//图片上传模块
			 ...
			 handleAvatarSuccess(fileNames) {
				 ...
				 if (fileNames && fileNames[0]) {
					 _this.ruleForm.brandImg = fileNames[0]
				 }
			 },
