<template>
	<div>
		<div class="claerfix text-right">
			<span class="text-md pull-left l-h36">颜色（公共数据）</span>
		...
		<div class="m-t-lg">
		    ...
		    <el-table-column
		      ...
		      <template slot-scope="scope">
		      	<div class="color-block" :style="{ background: scope.row.color }"></div></template>
		    </el-table-column>
		    <el-table-column
		      label="名称">
		...
		<div class="m-t-vl claerfix text-right">
			<span class="text-md pull-left l-h36">颜色（私有数据）</span>
		  <div class="w dis-inline m-l m-r-lg">
			...
			<auth perm="setting.public.data.project.update">
		  	<el-button @click="addForm('ruleForm')" type="primary">
					<i class="el-icon-plus m-r-xs"></i>添加颜色</el-button>
			</auth>
		</div>
		...
		<el-dialog class="ukao-dialog"
		  ...
		  <div>
		  	<el-form :model="ruleForm" ref="ruleForm" label-width="160px">
				  ...
				  <el-form-item label="选取颜色" prop="color" :rules="[{ required: true, message: '颜色不能为空'}
				  ]">
						<div class="l-h-0x">
		...
		methods: {
			...
			templateList (start){
				...
				this.$store.dispatch('getTemplateList', {
					uri : 'productColor/template/list',
					params: {
						startRow: start || 0,
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
						this.$store.dispatch('add', {
							uri: this.ruleForm.id ? 'productColor/update': 'productColor/add',
