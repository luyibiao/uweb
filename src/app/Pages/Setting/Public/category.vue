<template>
	<div>
		<a href="#public-data">去公共数据</a>
		<a href="#private-data">去私有数据</a>
		<div class="clearfix text-right">
			<span id="public-data" class="text-md pull-left l-h36">产品类别（公共数据）</span>
		...
		<div class="m-t-vl claerfix text-right">
			<span id="private-data" class="text-md pull-left l-h36">产品类别（私有数据）</span>
		  <div class="w dis-inline m-l m-r-lg">
			...
			<auth perm="setting.public.data.project.update">
		  	<el-button @click="addForm('ruleForm')" type="primary"><i class="el-icon-plus m-r-xs"></i>添加产品类别</el-button>
			</auth>
		</div>
		<div class="m-t-lg">
			<el-table class="ukao-el-table"
	...
	export default {
      ...
      addForm (formName){
          ...
          try {
			...
			detailForm (item){
				this.formTitle = '修改产品类别'
				this.$store.dispatch('post', {
					uri: 'productType/detail',
					params: {
						id: item.id
					}
				}).then((res) => {
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
						this.$store.dispatch('post', {
							uri: this.ruleForm.id ? 'productType/update': 'productType/add',
