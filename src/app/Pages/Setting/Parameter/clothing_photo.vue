<template>
  <div>
    <p class="m-t m-b text-black text-md font-bold">查看衣物照片设置</p>
    <div class="b-t b-b text-center padder-v">
      <span class="pull-left">微信订单详情查看照片</span>
  ...
  methods: {
    ...
    getStatus() {
      this.$store
        ...
        .then(res => {
          this.status = res.data.data.showWxProImg;
    ...
    saveStatus() {
      ...
      this.$store
        .dispatch("post", {
          uri: "mercCityParam/updateShowWxProImg",
          data: {
            showWxProImg: this.status
          }
        })
        .then(res => {
