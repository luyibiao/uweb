    ...
    <div class="m-b-md">
      <ul  class="uk-search">
        ...
        <li class="uk-search-item">
          ...
          <el-select class="uk-search-content w-sm"  v-model="tableData.isFinish" filterable clearable placeholder="全部">
            <el-option
          ...
          <template v-for="(item, index) in tableData.list">
            <tr class="table-content-border-bottom">
              <td>
                <div>{{item.code}}</div>
                <div>
                  <qrcode  :value="item.code"  v-if="item.code"  :options="{ size: 65 }"></qrcode>
                  <!--<barcode :value="item.code"  v-if="item.code" :options="{  displayValue: false, width:'1.5',  height:'25', margin:'0', fontSize:'20' }"></barcode>-->
                </div>
              </td>
              <td>
                <div>{{item.productCnt || 0}} / {{item.annexCnt || 0}}</div>
                <div class="text-muted">分拣：{{item.sortingCnt || 0}}</div>
              </td>
              <td>
              ...
              <td>
                <a class="text-primary" target="_blank" :href="'/product/batchIn/detail/'+item.id">详情</a>
              </td>
            </tr>
          </template>
        </tbody>
    ...
    data() {
      return {
        ...
        tabs:[
          ...
          {label:'送往工厂途中', name:'step2', value:2},
          {label:'到达工厂', name:'step3', value:3}
        ],
        step:{
          active: 'all'
        },
