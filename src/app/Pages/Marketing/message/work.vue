    ...
    <el-table
      ...
      <el-table-column label="员工" min-width="100">
        <template slot-scope="scope">
          <router-link class="text-primary" :to="`/worker/detail/${scope.row.workId}`">{{scope.row.workName}}</router-link>
        </template>
      </el-table-column>
      <el-table-column label="手机号" prop="workPhone" min-width="140"></el-table-column>
      ...
      <el-table-column label="消息类型" prop="msgTypeText"  min-width="140">
      </el-table-column>
      <el-table-column label="短信内容" prop="content" min-width="250">
      </el-table-column>
      <el-table-column label="发送状态"  min-width="120">
        <template slot-scope="scope">
          <span :class="scope.row.sendStatus ? 'text-success':'text-danger'">{{scope.row.sendStatus ? '发送成功' : '发送失败'}}</span>
        </template>
      </el-table-column>
    </el-table>
