<template>
	<div class="ukao-pt-md">
		...
		<div class="m-t-lg">
			<v-list-tab-nav v-bind:nav-key="'10'"></v-list-tab-nav>
		</div>
		<div class="m-t-lg">
        ...
        v-loading="loading"
          ...
          <template slot-scope="scope">
		    ...
		    <el-table-column
		      ...
		      label="支付方式">
		      <template slot-scope="scope">
						<div>{{ scope.row.payMethod | getValText('PAY_METHOD')}}</div>
		      </template>
		    </el-table-column>
				<el-table-column
		      label="支付场景">
		      <template slot-scope="scope">
						<div>{{ scope.row.payScene | getValText('ORIGIN') }}</div>
		      </template>
		    </el-table-column>
				<el-table-column
		      label="备注">
