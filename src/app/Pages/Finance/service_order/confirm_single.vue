<template>
<div class="ukao-pt-lg box-ft">
  <el-row>
    <el-col :lg="24" :md="24" :xl="24">
       <el-breadcrumb 
                       separator="/">
          <el-breadcrumb-item to="/finance/service_order/service_market">
            <i class="el-icon-arrow-left icon-smaller"></i>服务市场</el-breadcrumb-item>
          <el-breadcrumb-item>确认订购单信息</el-breadcrumb-item>
        </el-breadcrumb>
    </el-col>
  </el-row> 
  <el-row style="margin-top:20px;">
    <el-col :span="14" :offset="5">
      <div class="single_detail" v-loading="btnStatus.divloading">
        <div>
          <span style="margin-right:60px;">
            订购门店：
            <el-select v-model="storeId1" @change="storeChange">
              <el-option v-for="item in storeList" :key="item.id" :value="item.id" :label="item.name" ></el-option>
            </el-select>
          </span>
          <span>订购内容：{{obj.name}}</span>
        </div>
        <div class="single_table">
          <table>
            <tr>
              <td>服务内容</td>  
              <td>规格</td>
              <td>生效时间</td>
              <td>服务价格（元）</td>
            </tr>
            <tr style="border-top:1px solid #e5e5e5;border-bottom:1px solid #e5e5e5">
              <td>{{obj.serviceTypeName}}</td>  
              <td>{{obj.cnt}}个月</td>
              <td>订购成功后立即生效</td>
              <td>{{totalMoney.price}}元</td>
            </tr>
            <tr>
              <td colspan="3">
                <el-checkbox v-model="tongBi" :label="1" class="pull-left" :disabled="btnStatus.tongbiFlag || coinNum<=0">使用{{coinNum || 0}}个通洗币抵扣</el-checkbox>
                <p class="text-muted" style="margin-top:2px;"v-if="tongBi">(已使用通洗币抵扣，本次订购成功后无通洗币奖励)</p>
              </td>
              <td>
                <span >
                  <p>合计：{{totalMoney.price || 0}}元</p>
                  <p>通洗币抵扣：-{{coinPrice+".00" || 0}}元</p>
                </span>
              </td>
            </tr>
            <tr>
               <td colspan="2" >
                 <label>使用推广码：</label>
                 <el-input style="width:180px;" placeholder="填写好友的推广码" :disabled="tongBi" v-model="spreadCode"></el-input>
                  <!-- <input type="text" class="input" placeholder="填写好友的推广码"/> -->
               </td>
              <td colspan="2">
                <span style="display:inline-block;">
                  <p>应付款：<span style="color:#FF7012">￥<span style="font-size:20px;">{{obj.price}}元</span></span></p>
                  <p v-if="!tongBi && storeId1">现在订购，可获得{{queryCoinNum}}个通洗币奖励</p>
                  <el-button type="primary" style="width:100%;margin-top:7px;" @click="payBtn" :disabled="btnStatus.payBtnFlag" :loading="btnStatus.payLoading">立即付款</el-button>
                </span>
              </td>
            </tr>
          </table>
        </div>
      </div>
    </el-col>
  </el-row>

  <!--付款弹出框-->
   <el-dialog :visible.sync="dialogData.dialogvisible" :title="dialogData.diaTitle">
     <div class="dialog-content" v-if="dialogData.contentShow===1">
       <p>订购内容： {{obj.name}}</p>
       <p>订购门店： {{storeId1 | selectStore(storeList)}}</p>
       <p>应付金额： <span style="color:#FF7012;font-size:20px;">{{obj.price}}元</span></p>
       <div class="pay-type">
         <p>支付方式： </p>
         <div class="pay-type-type">
           <!-- {{'余额'+dialogData.mercPayInfo.balance}},{{'价格'+obj.price}} -->
            <el-radio-group v-model="dialogData.payWay" @change="payWayChange">
              <el-radio label="1" style="margin-bottom:10px;" :disabled="dialogData.disabledRadio">余额支付(剩余余额￥{{dialogData.mercPayInfo.balance}}元<label v-if="dialogData.mercPayInfo.balance<obj.price" class="text-muted">，余额不足请先充值</label>)</el-radio>
              <p><el-radio label="2" style="margin-bottom:10px;" :disabled="dialogData.disabledCode">扫码支付</el-radio></p>
              <!-- <el-radio label="3">汇款支付</el-radio> -->
          </el-radio-group>
         </div>
       </div>
       <div style="width:100%;height:1px;background-color:#e5e5e5;"></div>
       <div class="pay-detail">
         <div v-show="dialogData.payWay==='1'" style="width:80px;margin:20px auto">
           <el-button type="primary" @click="balanceBtn" :loading="btnStatus.balanceBtn">立即支付</el-button>
         </div>
         <div v-show="dialogData.payWay==='2'" style="margin-top:20px;">
           <div>
              <el-radio-group v-model="dialogData.codePay" @change="codePayFun">
                <el-radio-button :label="1">微信</el-radio-button>
                <el-radio-button :label="2">支付宝</el-radio-button>
              </el-radio-group>
         </div>
         <div class="code_pay">
             <div>
               <!-- <Qrcode  :options="{ size: 150 }"></Qrcode> -->
             <!-- <img :src="dialogData.codePay===1 ? '/static/img/marketing/coupon.png' : '/static/img/marketing/checked.png'"> -->
             <Qrcode v-if="!dialogData.orderQcode.loading" v-show="dialogData.orderQcode.type === 1" :value="dialogData.orderQcode.wxValue" :options="{ size: 150 }"></Qrcode>
             <Qrcode v-if="!dialogData.orderQcode.loading" v-show="dialogData.orderQcode.type === 2" :value="dialogData.orderQcode.zfbValue" :options="{ size: 150 }"></Qrcode>
              <!-- <img src="" v-if="dialogData.codePay===1">
              <img src="" v-else-if="dialogData.codePay===2"> -->
           </div>
           <div class="code_pay-way">
             <span v-if="dialogData.codePay===1"><img src="/static/img/finance/charge/wePay.png">微信支付</span>
             <span v-else="dialogData.codePay===2"><img src="/static/img/finance/charge/zhiPay.png">支付宝支付</span>
           </div>
           <p style="font-size:22px;">扫一扫完成支付</p>
         </div>
         </div>
         <div v-show="dialogData.payWay==='3'" class="remit">
           <div class="remit_content">
              <p>你需汇款<label style="color:#FF7012">{{obj.price}}元</label>至以下账户，汇款成功后上传凭证信息，审核通过后到账</p>
              <div class="remit-content_info">
                <table>
                  <tr>
                    <td>收款方户名：</td>
                    <td>{{dialogData.mercPayInfo.payment_name}}</td>
                  </tr>
                   <tr>
                    <td>收款方开户行：</td>
                    <td>{{dialogData.mercPayInfo.payment_bank_name}}</td>
                  </tr>
                   <tr>
                    <td>收款方账户：</td>
                   <td>{{dialogData.mercPayInfo.payment_bank_no}}</td>
                  </tr>
                </table>
              </div>
           </div>
         </div>
         <div v-if="dialogData.payWay==='3'" style="margin:30px 0px 10px auto;width:186px;" ><el-button type="primary" @click="remitBtn">已转账汇款，我要上传凭证</el-button></div>
       </div>
     </div>

     <div v-else-if="dialogData.contentShow===2" class="proof">
       <div class="make-info">
         <p class="text-muted">收款方信息</p>
         <div>
           <table>
             <tr>
               <td>户名：</td>
               <td>{{dialogData.mercPayInfo.payment_name}}</td>
             </tr>
              <tr>
               <td>开户行：</td>
               <td>{{dialogData.mercPayInfo.payment_bank_name}}</td>
             </tr>
              <tr>
               <td>账号：</td>
               <td>{{dialogData.mercPayInfo.payment_bank_no}}</td>
             </tr>
           </table>
           <!-- <p>户名：湖南友靠智能科技有限公司</p>
           <p>开户行：招商银行麓谷支行</p> -->
         </div>
       </div>
       <div class="payment">
         <p class="text-muted">付款信息</p>
         <div>
           <el-form :model="dialogData.payRuleForm" ref="payRuleForm" label-width="110px">
             <el-form-item label="付款户名：" :rules="[{required: true, message: '请输入户名'}]" prop="accountName">
               <el-input v-model="dialogData.payRuleForm.accountName" class="w-lg" ></el-input>
             </el-form-item>
             <el-form-item label="付款账号：" :rules="[{required: true, message: '请输入付款账号'}]" prop="accountNo">
               <el-input v-model="dialogData.payRuleForm.accountNo" class="w-lg"></el-input>
             </el-form-item>
             <el-form-item label="金额："><span>{{obj.price}}元</span></el-form-item>
             <el-form-item label="付款时间" :rules="[{required: true, message: '请填写付款时间'}]" prop="tradeTime">
               <span>
                  <el-date-picker
                    v-model="dialogData.payRuleForm.tradeTime"
                    type="date"
                    placeholder="选择日期"
                    class="w-lg"
                    :picker-options="pickerOptions">
                  </el-date-picker>
               </span>
             </el-form-item>
             <el-form-item>
               <el-button type="primary" @click="onPaySubmit('payRuleForm')" :loading="btnStatus.remitBtnStatus">确认提交</el-button>
             </el-form-item>
           </el-form>
         </div>
       </div>
     </div>

     <div class="" v-else-if="dialogData.contentShow===3">
       <p>凭证已提交，友靠将在3个工作日内完成审核，通过后自动完成订购</p>
       <div class="pull-right" style="margin:20px 0px;">
          <el-button type="primary" @click="myKnow">我知道了</el-button>
       </div>
     </div>
   </el-dialog>
</div>  
</template>
<script>
import Qrcode from "@xkeshi/vue-qrcode";
import msg from '@/utils/msg'
import calculation from '@/utils/helpers/calculation'
export default {
  components: {
    Qrcode
  },
  data () {
    return {
      tongBi:false,//是否勾选了通洗币抵扣
      spreadCode:undefined,//推广码,
      coin:"",//是否勾选抵扣标识
      coinNum:'',//通洗币总数
      dialogData:{
        dialogvisible:false,//弹出框开关
        diaTitle:'选择付款方式',//弹出框标题
        payWay:"1",//付款方式
        codePay:1,//扫码付款方式
        contentShow:1,//弹出框内容显示
        disabledRadio:false,//余额不足单选框是否禁止
        disabledCode:false,//应付款为0时禁止选择扫码支付
        payRuleForm:{
          orderId:'',//订单id
          tradeType:'0201',
          amount:"",//交易金额
          payMethod:7,
          accountName:'',//户名
          accountNo:"",//账号
          tradeTime:'',//付款时间
          incrementType:2
        },
       mercPayInfo:{},
       orderQcode:{
           type: 1, //支付类型
           loading: true, //开启支付状态
           wxValue: "", //付款码值
           zfbValue: "", //付款码值
          incrementSeq: "", //订单id
       },
       orderQcodeTrade:{
         wxTradeSeq:'',
         zfbTradeSeq:""
       }
      },
      btnStatus:{
        tongbiFlag:false,//通洗币是否抵扣开关
        payBtnFlag:true,//立即支付按钮禁止开关
        payLoading:false,
        balanceBtn:false,//余额正在支付
        divloading:true,// 读取数据
        remitBtnStatus:false,//点击汇款按钮状态
      },
      totalMoney:{
        money:"",
        // coinPrice:'',
      },
      obj:{},//获取订单详情
      queryCoin:{},//得到通洗币奖励对象
      queryCoinNum:'',//得到通洗币奖励数
      storeId:"",//获取传过来的门店id
      storeId1:"",//门店下拉框的id
      monthId:"",//选择的月份id
      storeList:[],//门店列表
      timer:'',//定时器
      pickerOptions: {
        disabledDate(time) {
          return time.getTime() > Date.now();
        },
      }
    }
  },
  created () {
    this.getId(this.getStroelist());
    this.getDetail();
    // this.spreadCode=undefined;
    this.coin=undefined;
    this.coinPrice=0;
  },
  watch: {
    "dialogData.dialogvisible":'dialogvisibleChange',
    storeId1(end,start){
      if(this.storeId1!==""){
       this.btnStatus.payBtnFlag=false;
      }
    },
    tongBi:'changTongbi',
    spreadCode:'getCoinNum'
  },
  methods: {
    storeChange(){
      this.queryAward(this.storeId1);
    },

     //  获取门店管理传过来的id值
    getId(callback){
       this.monthId=this.$route.query.monthId;
      if(this.$route.query.storeId && this.$route.query.storeId!==""){
        this.storeId=this.$route.query.storeId;
        this.queryAward(this.storeId);
      }
      if(typeof callback==="function"){
        callback();
      }
    },

    //门店列表
    getStroelist(){
      this.$store.dispatch('post',{
        uri:"storeInfo/list",
        data:{}
      }).then(res=>{
        let _this=this;
        if(this.storeId!==""){
          _this.storeId1=parseInt(_this.storeId);
        }
         this.storeList=res.data.data.list;
      })
    },

    queryAward(id){
      //得到奖励通洗币数
      this.$store.dispatch('post',{
        uri:'mercCoinBill/queryAward',
        data:{
          storeId:id,
          incrementId:this.monthId
        }
      }).then(res=>{
        this.queryCoin=res.data.data;
        this.getCoinNumFlag=true;
        this.getCoinNum();
        // this.queryCoinNum=obj;
      })
    },

    getCoinNum(){
      if(!this.getCoinNumFlag){
        return;
      }
      //筛选通洗币奖励数
      let _this=this
      let obj= this.queryCoin
      if(obj.buyNum<=0){
        //首次购买
        if(this.spreadCode){
          //是否填写了推广码
          _this.queryCoinNum=obj.incrementItemRule.firstInvitedCoin
        }
        else{
          _this.queryCoinNum=obj.incrementItemRule.giveCnt
        }
      }
      else{
        //不是首次购买
        if(obj.earlyDay<=0){
          //未提前购买
          _this.queryCoinNum=0
        }
        else{
          //提前购买
          if(this.spreadCode){
            //是否填写推广码
            _this.queryCoinNum=obj.incrementItemRule.renewInvitedCoin;
          }
          else{
            _this.queryCoinNum=obj.incrementItemRule.earlyCoin
          }
        }
      }
    },

     //获得订单详情
    getDetail(){
      this.$store.dispatch('post',{
       uri:"incrementItem/service/detail",
       data:{
         id:this.monthId
       }
     }).then(res=>{
       let _this=this;
       this.coinNum=res.data.data.coin;
       res.data.data.incrementItemRule.price=calculation.accDiv(res.data.data.incrementItemRule.price,100);
       this.obj=res.data.data.incrementItemRule;
       this.totalMoney={...this.obj};
       this.btnStatus.divloading=false;
        if(this.obj.useCoin===0){
         _this.btnStatus.tongbiFlag=true;
       }
       else{
         _this.btnStatus.tongbiFlag=false;
       }
     })
    },

    //切换微信支付宝
     codePayFun(){
      this.dialogData.orderQcode.type=this.dialogData.codePay;
    },

    //支付方式选择
    payWayChange(){
      clearInterval(this.timer);
      if(this.dialogData.payWay==='2'){
         if (this.timer) clearInterval(this.timer);
         this.timer = setInterval(_ => {
        this.tradeStatus(this.dialogData.mercPayInfo.seq);
      }, 3000);
      }
    },

    //订单查询状态
    tradeStatus(seq){
      this.$store.dispatch('post',{
        uri:"tradePay/queryStatusBySeq",
        data:{
          seq:this.dialogData.orderQcode.type===1 ? this.dialogData.orderQcodeTrade.wxTradeSeq : this.dialogData.orderQcodeTrade.zfbTradeSeq 
        }
      }).then(res=>{
        if(res.data.data.status===1){
          clearInterval(this.timer);
          this.$router.push({path:'/finance/service_order/buy_record'})
        }
      })
    },

    //得到二维码
     getQRValue(incrementSeq) {
      // wx
      this.$store
        .dispatch("post", {
          uri: "tradePay/increment/wxPay",
          data: {
            incrementSeq
          }
        })
        .then(res => {
          this.dialogData.orderQcode.loading=false;
          // this.order.loading = false;
          this.dialogData.orderQcode.wxValue = res.data.data.codeUrl;
          this.dialogData.orderQcodeTrade.wxTradeSeq=res.data.data.tradeSeq
        });
      // 支付宝
      this.$store
        .dispatch("post", {
          uri: "tradePay/increment/aliPay",
          data: {
            incrementSeq
          }
        })
        .then(res => {
          this.dialogData.orderQcode.zfbValue = res.data.data.codeUrl;
          this.dialogData.orderQcodeTrade.zfbTradeSeq=res.data.data.tradeSeq;
        });
    },

    //点击余额支付立即支付
    balanceBtn(){
      this.btnStatus.balanceBtn=true;
      this.$store.dispatch('post',{
        uri:"tradeIncrement/pay",
        forceResolve:true,
        data:{incrementSeq:this.dialogData.mercPayInfo.seq}
      }).then(res=>{
        if(res.data.httpCode===200){
         msg.success(res.data.msg);
        this.$router.push({ path: '/finance/service_order/buy_record'})
        }
        else{
           msg.error(res.data.msg);
           this.btnStatus.balanceBtn=false;
        }
      })
    },

    //创建订单
     payBtn(){
      this.dialogData.orderQcode.loading=true;
      this.btnStatus.payLoading=true
      this.$store.dispatch('post',{
        uri:"tradeIncrement/service/recharge/add",
        forceResolve:true,
        data:{
          storeId:this.storeId1,
          ruleId:this.monthId,
          spreadCode:this.spreadCode,
          coin:this.coin
        }
      }).then(res=>{
         let _this=this
        if(res.data.httpCode===200){
          let price=this.obj.price;
          price=calculation.accMul(price,100);
          console.log(price,'price',res.data.data.balance,'balance')
          if(price>res.data.data.balance){
            _this.dialogData.disabledRadio=true;
            _this.dialogData.payWay='2'
            _this.payWayChange();
          }
          if(price===0){
            _this.dialogData.disabledCode=true;
          }
          res.data.data.balance=calculation.accDiv( res.data.data.balance,100);
          _this.dialogData.mercPayInfo=res.data.data;
          _this.dialogData.payRuleForm.orderId=_this.dialogData.mercPayInfo.incrementId;
          _this.btnStatus.payLoading=false
          _this.dialogData.dialogvisible=true;
          if(this.obj.price || this.obj.price>0){
             _this.getQRValue(res.data.data.seq)
          }
        }
        else{
          _this.btnStatus.payLoading=false
          msg.error(res.data.msg);
        }
      })
    },

     //点击我要上传凭证
    remitBtn(){
      this.dialogData.diaTitle="线下转账凭证"
      this.dialogData.contentShow=2;
    },

     //提交转账凭证
    onPaySubmit(formName){
       this.$refs[formName].validate(valid => {
        if (valid) {
          let amount=this.obj.price;
          this.dialogData.payRuleForm.amount=calculation.accMul(amount,100);
          this.dialogData.payRuleForm.tradeTime=this.$options.filters.getDate(this.dialogData.payRuleForm.tradeTime);
          this.dialogData.payRuleForm.accountNo=this.dialogData.payRuleForm.accountNo.trim().replace(/\s/g,"");
          this.btnStatus.remitBtnStatus=true;
          // console.log(this.obj.price,'....',this.dialogData.payRuleForm.amount)
          this.$store.dispatch('post',{
            uri:"tradeAudit/merc/recharge",
            data:this.dialogData.payRuleForm,
            forceResolve:true,
          }).then(res=>{
            if(res.data.httpCode===200){
               this.dialogData.contentShow=3;
               this.dialogData.diaTitle="转账汇款凭证"
               msg.success('操作成功');
               this.btnStatus.remitBtnStatus=false;
            }
            else{
               msg.error(res.data.msg);
               this.btnStatus.remitBtnStatus=false;
            }
          })
        } 
      });
    },

     // 点击我知道了按钮
    myKnow(){
      this.dialogData.dialogvisible=false;
    },


    //弹出框隐藏所做操作
    dialogvisibleChange(endVal,startVal){
       if(!endVal){
        //  this.dialogData.diaTitle="选择付款方式";
        //  this.dialogData.contentShow=1;
        //  this.dialogData.payRuleForm={};
        // this.dialogData.payRuleForm=""
        //  this.dialogData.payWay='1';
        //  this.dialogData.codePay=1;
        //  clearInterval(this.timer);
          this.$router.push({ path: '/finance/service_order/buy_record'})
       }
    },

    //通洗币勾选所做操作
    changTongbi(endval,startVal){
      if(this.tongBi){
        this.spreadCode=undefined;
        this.spreadCode="";
        this.coin=1;
        this.coinPrice=this.coinNum*1;
        this.obj.price=this.obj.price- this.coinPrice;
        if(this.obj.price<0){
          this.obj.price=0;
          this.coinPrice=this.totalMoney.price;
        }
      }
      else{
        this.obj.price=this.obj.price+this.coinPrice;
        this.coin=undefined;
         this.coinPrice=0;
      }
    },
  },
  filters: {
    selectStore(id,Arr){
      if(Arr.length===0){
        return;
      }
      for(let i=0;i<Arr.length;i++){
        if(id===Arr[i].id){
          return Arr[i].name;
          break;
        }
      }
    }
  }
}
</script>

<style lang="less" scoped>
.w-lg{
  width: 70%;
}
.single_detail{
  width: 100%;
}
.single_table table{
  width: 100%;
  margin-top: 20px;
    // border-collapse: collapse;
}
.single_table table tr:nth-child(odd){
  background-color: #f5f5f5;
}
.single_table table tr:nth-child(even){
  background-color: #ffffff;
}
.single_table table tr:last-child{
  background-color: #f5f5f5;
  border-top: 1px solid #e5e5e5;
  height: 126px;
}
.single_table table tr td{
  width: 25%;
  height: 50px;
  vertical-align: middle;
  // background-color: pink;
}
.single_table table tr>:last-child,.single_table table tr>:first-child{
  padding: 0px 20px;
}
.single_table table tr td:nth-child(2),.single_table table tr td:nth-child(3){
  text-align: center;
}
.single_table table tr td:last-child{
  text-align: right;
}
.single_table table tr:nth-child(3){
  height: 80px;
}
.dialog-content>p+p{
  margin-top: 10px;
}
.pay-type{
  position: relative;
  margin: 10px 0px;
  height: 105px;
}
.pay-type-type{
  position: absolute;
  top: 0;
  left: 70px;
}
.pay-detail{
  // padding: 20px 20px;
  width: 100%;
  // min-height: 40px;
  // border: 1px solid #e5e5e5;
  // border-radius: 5px;
}
 .code_pay{
    width: 250px;
    margin: 25px auto 0px;
    // background-color: pink;
    text-align: center;
    img{
      display: inline-block;
    }
  }
.code_pay .code_pay-way{
   margin-bottom: 30px;
    img{
      width: 25px;
      vertical-align: middle;
    }
    span{
      cursor: pointer;
    }
}
.remit{
  margin-top: 40px;
  // margin-bottom: 30px;
  min-height: 40px;
  border: 1px solid #e5e5e5;
  border-radius: 5px;
}
.remit .remit_content{
  margin:15px;
}
.remit .remit-content_info{
  margin-top: 20px;
  margin-left: 40px;
  table tr{
    height: 30px;
    font-size: 14px;
    font-weight:bold; 
  }
  table tr td:first-child{
    text-align: right;
  }
}
.proof .make-info{
  border-bottom: 1px dashed	#e5e5e5;
  padding-bottom: 30px;
  margin-bottom: 10px;
  &>div{
    width: 70%;
    margin: 0 auto;
  }
   table tr{
    height: 30px;
    font-size: 14px;
    font-weight:bold; 
  }
  table tr td:first-child{
    text-align: right;
  }
}
.proof .payment{
  &>div{
    width: 70%;
    margin: 0 auto;
  }
}
//  .input::-webkit-input-placeholder { color: #dcdfe6;}
//  .input:-moz-placeholder { color: #dcdfe6;}
//  .input:-ms-input-placeholder { color: #dcdfe6;}​
.input{
  padding: 3px 10px;
  border:1px solid #e5e5e5;
  width:55%;
  height:35px;
  outline: 0;
  font-size: 12px;
  transition: border-color .2s cubic-bezier(.645,.045,.355,1);
}
.input:focus{
  border: 1px solid #74b1e5;
}
</style>

