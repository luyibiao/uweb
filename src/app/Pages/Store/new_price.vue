<template>
  <div>
    <el-breadcrumb class="link-back" separator="/">
      <el-breadcrumb-item to="/store/price">
        <i class="el-icon-arrow-left icon-smaller"></i>产品价格</el-breadcrumb-item>
      <el-breadcrumb-item>新建产品</el-breadcrumb-item>
    </el-breadcrumb>
    <div class="m-l-xxl  m-t-lg m-r-xxl">
      <el-form :model="ruleForm" ref="ruleForm" label-width="100px" class="clearfix">
        <div class="pull-left" style="width:40%;">
          <el-form-item class="w-lg" label="所属门店" prop="storeId" :rules="[{ required: true, message: '请选择门店' }]">
            <el-select v-model="ruleForm.storeId" clearable placeholder="请选择/搜索" @change="onSelectedStore">
          ...
          <el-form-item class="w-lg" label="所属类别" prop="typeId" :rules="[{ required: true, message: '请选择所属类别'}]">
            <el-select v-model="ruleForm.typeId" clearable placeholder="请选择所属类别" @change="selectedProductType">
              <el-option :label="item.name + '(' + item.businessName + ')'" :value="item.id" :key="item.id" v-for="item in productTypeList"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item class="w-lg" label="计价方式" prop="unit" :rules="[{ required: true, message: '请选择计价方式' }]">
          ...
          <div class="price-item" v-if="ruleForm.priceList && ruleForm.priceList.length > 0">
            <el-form-item class="w-fohg" :label="item.name" :key="item.index" v-for="(item, index) in ruleForm.priceList" :prop="'priceList.' + index + '.showPrice'" :rules="[
          ...
          <!-- <el-form-item class="w-lg" label="普洗" prop="name" :rules="[
              { required: true, message: '请输入产品名称', trigger: 'blur' },
              { min: 1, max: 8, message: '长度在1到8个长度' }
            ]">
            ...
            <el-form-item class="w-lg" label="单烫" prop="name" :rules="[
              { required: true, message: '请输入产品名称', trigger: 'blur' },
              { min: 1, max: 8, message: '长度在1到8个长度' }
            ]">
              <el-input v-model="ruleForm.name">
            ...
            <el-form-item class="w-lg" label="护理" prop="name" :rules="[
              { required: true, message: '请输入产品名称', trigger: 'blur' },
              { min: 1, max: 8, message: '长度在1到8个长度' }
            ]">
              <el-input v-model="ruleForm.name">
          ...
          <el-form-item label="折扣" prop="saleType" :rules="[
              { required: true, message: '请选择折扣' }]
            ">
            <el-radio-group v-model="ruleForm.saleType">
              <el-radio :key="item.id" v-for="item in saleTypeList" :label="item.val">{{item.showText}}</el-radio>
            </el-radio-group>
          ...
          <div class="price-item" v-if="ruleForm.saleType == 2">
            <el-form-item class="w-xxl" label="折扣比例" prop="discount" :rules="[
              ...
              <el-input v-model.number="ruleForm.discount" type="number" placeholder="请输入折扣比例"></el-input>
              <span>%</span>
            </el-form-item>
          </div>
          <el-form-item label="输送类别" prop="tranType" :rules="[
              { required: true, message: '请选择输送类别' }
            ]">
            <el-radio-group v-model="ruleForm.tranType">
              <el-radio :label="item.val" :key="item.id" v-for="item in tranTypeList">{{item.showText}}</el-radio>
            </el-radio-group>
            <p style="line-height: 0px;" class="text-xs text-muted">滑杆/输送线</p>
          ...
          <el-form-item label="自定义数量" prop="isCustomNum">
            <el-switch active-color="#13ce66" v-model="ruleForm.isCustomNumVal" inactive-color="#dcdcdc" :width="74" on-text="是" off-text="否">
            </el-switch>
            <p class="text-muted text-xs">入库时是否可以自定义产品数量</p>
          </el-form-item>
        </div>
        <div class="pull-left" style="width:60%;">
          <el-form-item label="可使用图片" prop="imgPath">
            <uploadImg v-model="availableImgItem" 
                       :keywords="ruleForm.name" 
                       @selected="selectedAvailableImgs"></uploadImg>
          </el-form-item>
          <el-form-item label="自定义图片" prop="imgPath">
          ...
          <el-form-item>
            <router-link to="/store/price">
              <el-button>取消</el-button>
            </router-link>
            <el-button type="primary" @click="submitForm('ruleForm')" :loading="saveLoading">{{saveLoading?'保存中':'保存'}}</el-button>
          </el-form-item>
        </div>
      </el-form>
    </div>
...
export default {
  ...
  methods: {
    ...
    getProductServiceList() {		//服务及价格
      this.$store.dispatch('getList', {
        ...
        this.productServiceList.forEach(function(item, index) {
          tmpPriceList.push(
          {index:index, serviceId: item.id ,name: item.name, price:'', showPrice: '', vipPrice: '', showVipPrice: '',  isShow:0, unit:item.unit}
          )
        })
    ...
    handleSaveSuccess(att) { //保存成功后调用
      let tags = JSON.parse(att.data.tags) || [];
      if(tags.indexOf(this.ruleForm.name) < 0){   //判断产品名是否在标签中   不在则手动加入并保存
        tags.push(this.ruleForm.name);
        this.updateTags(att.data.id,tags);
      }
      this.saveForm();//保存数据
    },
    handleAvatarSuccess(fileNames) {  //图片上传成功后回调函数
      let _this = this;
    ...
    saveForm(){ //保存表单数据
      this.$store.dispatch('post', {
        ...
        if (res.data.httpCode == 200) {
          ...
          this.$router.push({ path: '/store/price' })
        } else {
          msg.error(res.data.msg);
        }
      })
    },
    submitForm(name) { //保存按钮    
      let _this = this
      //校验成功则先图片文件上传
      this.$refs[name].validate((valid) => { 
        if (valid) {
          ...
          if(this.$refs.upload.imgPath){ //是否使用上传图片
            this.$refs.upload.$children[0].submit();  //是则上传图片后保存 
          }else{  //否则直接保存
            if(this.availableImgItem ){ //是否使用了可选择的图片 
            ...
            this.ruleForm.imgPath = this.availableImgItem ? this.availableImgItem.path : "";
            this.saveForm();
          }
        } else {
          return false
